...
When PyTorch is available, the lab exercises:
- Plain tensor roundtrip: [[1.0, -2.0], [3.5, 0.25]], float32
- Primitive checkpoint: step=7, name='tiny-checkpoint', weights=[1.25, -0.5]
- state_dict keys: linear.weight, linear.bias
- Model reconstruction: missing_keys=[], unexpected_keys=[]
- Benign custom object restricted load: expected_error
- Trusted local full-object load: label='local-only', count=3
- Malformed checkpoint rejection: expected_error
- CPU map_location: device='cpu'
...
- PyTorch was not available; torch-dependent cases honestly classified as framework_skip.
- Environment marker, documentation checks, and artifact checks completed.
